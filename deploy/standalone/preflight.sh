...
check_path "$GOTTH_BB_SMTP_CREDENTIAL_KEY_FILE" 0:65532:440 file
[ "$(wc -c <"$GOTTH_BB_SMTP_CREDENTIAL_KEY_FILE" | tr -d ' ')" -eq 32 ] || fail "SMTP credential key must contain exactly 32 bytes"
...
$GOTTH_BB_SMTP_CREDENTIAL_KEY_FILE
...
unset APP_ENV PUBLIC_BASE_URL BASE_PATH OIDC_ISSUER_URL OIDC_CLIENT_ID AUTHENTIK_CONTROL_TOKEN_FILE AUTHENTIK_CONTROL_OBJECTS_FILE AUTHENTIK_CONTROL_SOCKET INVITATION_FINGERPRINT_KEY_FILE SMTP_CREDENTIAL_KEY_FILE REGISTRATION_URL SMTP_HOST SMTP_PORT SMTP_USERNAME SMTP_FROM SMTP_TLS_MODE SMTP_TIMEOUT SMTP_PASSWORD_FILE
set -a
. "$GOTTH_BB_ENV_FILE"
set +a
...
[ "${SMTP_CREDENTIAL_KEY_FILE-}" = "/run/secrets/smtp_credential_key" ] || fail "app SMTP credential key path differs"
...
[ -z "$SMTP_HOST$SMTP_PORT$SMTP_USERNAME$SMTP_FROM$SMTP_TLS_MODE$SMTP_TIMEOUT$SMTP_PASSWORD_FILE" ] || fail "legacy app SMTP tuple must be empty"
[ -z "$GOTTH_BB_SMTP_HOST$GOTTH_BB_SMTP_PORT$GOTTH_BB_SMTP_USERNAME$GOTTH_BB_SMTP_FROM$GOTTH_BB_SMTP_TLS_MODE$GOTTH_BB_SMTP_TIMEOUT_SECONDS" ] || fail "legacy Authentik SMTP tuple must be empty"
[ "$GOTTH_BB_SMTP_USE_TLS:$GOTTH_BB_SMTP_USE_SSL" = false:false ] || fail "legacy Authentik SMTP TLS flags must be false"
