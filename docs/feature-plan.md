...
1. **B1-09-00 — contract, Authentik mechanism, and permission admission.**
   ...
   invitations, creator-scoped initial permissions, group membership, API
   tokens, and object permissions.
...
3. **B1-09-02 — Authentik enrollment and isolated control gateway.** Extend the
   ...
   the upstream invitation `send_email` and usable non-superuser
   self-token-creation excesses against a disposable Authentik 2026.5.2
   instance, then prove the gateway makes both unreachable from Board.
4. **B1-09-03 — registration, approval, invitation, and suspension
   ...
   suspension/reinstatement and bounded finite-expiry reconciliation,
   pending-orphan adoption, idempotency, audit redaction, and failure/retry evidence.
