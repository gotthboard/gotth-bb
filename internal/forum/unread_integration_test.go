...
func TestMarkTopicReadTransactionsOnPostgreSQL17(t *testing.T) {
	...
	t.Run("server boundary retries and publication exclusion", func(t *testing.T) {
		...
		otherReply, err := CreateReply(ctx, connection, testPublicationPolicy, testDestinationPolicy, other, publicTopic.id, rootID, "new other reply")
		...
		ownReply, err := CreateReply(ctx, connection, testPublicationPolicy, testDestinationPolicy, reader, publicTopic.id, rootID, "own reply")
	...
	t.Run("post committed after statement snapshot remains unread", func(t *testing.T) {
		...
		published, err := CreateReply(ctx, connections[5], testPublicationPolicy, testDestinationPolicy, other, topic.id, topic.postIDs[0], "post after snapshot")
	...
	t.Run("publishing creates no marker", func(t *testing.T) {
		...
		created, err := CreateTopic(ctx, connection, testPublicationPolicy, testDestinationPolicy, reader, "public", "No automatic marker", "body")
		if err != nil {
			t.Fatalf("CreateTopic() returned error: %v", err)
		}
		assertNoMarker(t, ctx, connection, readerID, created.TopicID)
		if _, err := CreateTopic(ctx, connection, testPublicationPolicy, testDestinationPolicy, reader, "read-only", "Denied marker", "body"); !errors.Is(err, ErrPublishingDenied) {
