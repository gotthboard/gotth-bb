...
func TestAccountAdministrationProjectionQueriesFenceAuthorityAndBindLimits(t *testing.T) {
	...
	for _, test := range []struct {
		...
		},
		{
			...
			required: []string{"actor AS MATERIALIZED", "forum_user.muted_until IS NULL OR forum_user.muted_until <= $2", "target AS MATERIALIZED", "FROM target", "JOIN LATERAL", "LIMIT $5", "EXISTS", "membership.user_id = $3", "membership.group_id = forum_group.id"},
		},
		{
