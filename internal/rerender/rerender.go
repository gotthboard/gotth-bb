...
const selectStalePostsSQL = `SELECT id, markdown_source
FROM public.posts
...
// by the schema and batch limit. Output is one constant-size line per batch.
...
// auxiliary/returned space is O(b+n+h), Omega(1), bounded by b <= 100 and the
...
func runBatch(ctx context.Context, database database, batchSize int) (result BatchResult, retryable bool, resultErr error) {
	...
	rows, err := tx.Query(ctx, selectStalePostsSQL, contentrender.RendererVersion, int32(batchSize))
	if err != nil {
	...
	for rows.Next() {
		var candidate post
		if err := rows.Scan(&candidate.id, &candidate.markdown); err != nil {
			rows.Close()
		...
		if err != nil {
			rows.Close()
			return BatchResult{}, false, fmt.Errorf("render stale post: %w", err)
		}
		candidate.html, _, err = rendered.PersistenceValues()
		if err != nil {
			rows.Close()
			return BatchResult{}, false, fmt.Errorf("read rendered stale post: %w", err)
		}
	...
	if len(posts) == 0 {
		...
		tag, err := tx.Exec(ctx, completeMigrationSQL, contentrender.RendererVersion)
		if err != nil {
	...
	for _, candidate := range posts {
		tag, err := tx.Exec(ctx, updateRenderedPostSQL, candidate.html, contentrender.RendererVersion, candidate.id)
		if err != nil {
