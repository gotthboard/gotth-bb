...
const testRuntimeGrants = requiredRuntimeGrantsStatements + "\n"
...
var testDeploymentFiles = map[string]struct {
	...
	"deploy/postgresql/restore-logical.sh":             {data: "#!/bin/sh\nexit 0\n", mode: 0o755},
...
func TestBuildProducesDeterministicAtomicArtifact(t *testing.T) {
	...
	wantEntries := []string{
		...
		root + "/deploy/postgresql/restore-logical.sh",
	...
	wantCalls := []string{
		...
		"git show " + testCommit + ":deploy/postgresql/restore-logical.sh",
...
func TestCanonicalRuntimeGrantsRejectsMalformedData(t *testing.T) {
	...
	invalid := [][]byte{
		...
		[]byte("GRANT TO :\"runtime_role\"; GRANT TO :\"runtime_role\"; GRANT TO :\"runtime_role\";\n"),
	...
	invalidPolicies := map[string]string{
		"missing initial revoke": strings.Replace(testRuntimeGrants,
			`REVOKE ALL PRIVILEGES ON ALL TABLES IN SCHEMA public FROM :"runtime_role";`+"\n", "", 1),
		"table-wide users update": strings.Replace(testRuntimeGrants,
			`UPDATE (display_name, email, avatar_url, role, suspended_at,`,
			`UPDATE`, 1),
		"unexpected table grant": strings.Replace(testRuntimeGrants,
			`    public.email_test_state`+"\n"+`TO :"runtime_role";`,
			`    public.email_test_state,`+"\n"+`    public.unexpected`+"\n"+`TO :"runtime_role";`, 1),
		"unexpected sequence grant": strings.Replace(testRuntimeGrants,
			`    public.users_id_seq`+"\n"+`TO :"runtime_role";`,
			`    public.users_id_seq,`+"\n"+`    public.unexpected_id_seq`+"\n"+`TO :"runtime_role";`, 1),
	}
	for name, grants := range invalidPolicies {
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			if _, err := canonicalRuntimeGrants([]byte(grants)); err == nil {
				t.Fatal("canonicalRuntimeGrants() accepted a widened runtime policy")
			}
		})
	}
}
