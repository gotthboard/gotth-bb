...
if [ -n "$(git status --porcelain=v1)" ]; then
  printf '%s\n' 'population evidence requires a clean committed source tree' >&2
  exit 2
fi
...
grep -F 'population_migration rows=25000 preflight_batches=251 preflight_transactions=252 preflight_round_trips=254 mutation_batches=250' "$test_log" >/dev/null
grep -F 'current=25000 converted=25000 completed=true validated=true' "$test_log" >/dev/null
