...
func TestApplyReleaseUpgradesPopulatedAlpha2OnPostgreSQL17(t *testing.T) {
	...
	if migrationCount != 13 || postCount != 1 || !rendererComplete || !projectionComplete {
		t.Fatalf("upgraded Alpha.2 state = (migrations %d, posts %d, renderer %t, projection %t), want 13/1/true/true", migrationCount, postCount, rendererComplete, projectionComplete)
	}
}
